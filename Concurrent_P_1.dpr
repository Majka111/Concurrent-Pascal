...
{####}
  ...
  BEGIN
    ...
    WITH OPS[T.) DO
      ...
      begin
       ERR:=SUPPRESS;
        writeln('This Param is not iniatialized' );
        readln;
      end;
...
{########}
  ...
  BEGIN
    ...
    WITH SPELLING_TABLE[SPIX] DO
      IF ACCESS IN OP_ACCESS THEN
       BEGIN
        NAME_ENTRY:=ENTRY;
        CASE ACCESS OF
          GENERAL: ;
          FUNCTIONAL: RESULT:=TRUE;
          INTERNAL: IF LEVEL<SYSCOMP_LEVEL
              THEN ERR:=TRUE;
          QUALIFIED: BEGIN
            COMP:=TRUE; PUSH {WITH TEMP};
            WITH DISPLAY[LEVEL.).LEVEL_ENTRY^ DO
            BEGIN
...
function IntToCHK_MODE(I:integer):INPUT_MODE;
{
CLASS1_MODE=1;     MONITOR1_MODE=2;    PROCESS1_MODE=3;    PROC1_MODE=4;
PROCE1_MODE=5;     FUNC1_MODE=6;       FUNCE1_MODE=7;      PROGRAM1_MODE=8;
RECORD_MODE=9;
...
begin
case I of
1: result:=CLASS1_MODE;
2: result:=MONITOR1_MODE;
3: result:=PROCESS1_MODE;
4: result:=PROC1_MODE;
5: result:=PROCE1_MODE;
6: result:=FUNC1_MODE;
7: result:=FUNCE1_MODE;
8: result:=PROGRAM1_MODE;
9: result:=RECORD_MODE
else
 writeln ('CHK MODE neinicializován');
...
{####################}
  ...
  BEGIN
    ...
    chk_MODE:=IntToCHK_MODE(CHK_MODE1);
    PUSH_LEVEL(CHK_MODE{CHK_MODE});
    PARM_NUMBER:=0
  END;
      ...
      CASE CHK_MODE OF
        MONITOR1_MODE,PROCESS1_MODE,CLASS1_MODE:
          IF NOT(KIND IN SMALLS) THEN
            IF KIND=SYSCOMP_KIND THEN
              IF SMODE=MONITOR2_MODE
              THEN {OK}
                ELSE
                IF (SMODE=CLASS2_MODE) AND (CHK_MODE=CLASS1_MODE)
                 THEN {OK}
                ELSE
                  ERROR(PARM1_ERROR)
             ELSE ERROR(PARM2_ERROR);  {KIND<>SYSCOMP_KIND}
        PROC1_MODE,FUNC1_MODE: ;
        PROCE1_MODE,FUNCE1_MODE: BEGIN
          UNPACK(INHERITANCE,INHERIT);
          IF QUEUE_LEGACY IN INHERIT THEN
            ERROR(PARM4_ERROR)
        END;
        PROGRAM1_MODE:
          IF KIND IN ACTIVES THEN
            ERROR(PARM5_ERROR)
      END
  END;
...

{
  ...
  VAR C:CONTEXT_KIND;
  BEGIN
    IF CHK_MODE IN NONVARPARMS
      THEN ERROR(PARM7_ERROR);
    IF UNIVERSAL THEN BEGIN
...
case MODE of
 ...
 SCONST_MODE:result:=11
  ...
  writeln ('Int to MODE není inicializován');
 end;
end;
...
case I of
 ...
 11: result:=SCONST_MODE
  ...
  writeln ('MODE nedefinován');
end;
end;

function IntToKIND(I:integer): TYPE_KIND;
{
INT_KIND=0;        REAL_KIND=1;        BOOL_KIND=2;        CHAR_KIND=3;
ENUM_KIND=4;       SET_KIND=5;         STRING_KIND=6;      PASSIVE_KIND=7;
POINTER_KIND=8;    QUEUE_KIND= 9;      GENERIC_KIND=10;    UNDEF_KIND=11;
...
begin
 ...
 4: result:=ENUM_KIND;
 5: result:=SET_KIND;
 6: result:=STRING_KIND;
 7: result:=PASSIVE_KIND;
 8: result:=POINTER_KIND;
 9: result:=QUEUE_KIND;
 10: result:=GENERIC_KIND;
 11: result:=UNDEF_KIND;
 12: result:=SYSCOMP_KIND;
 13: result:=ROUTINE_KIND;
 14: result:=ACTIVE_KIND
 else
  writeln('KIND nedefinován');
 end;
end;
    ...
    WITH T^ DO
      CASE KIND OF
       ...
       GENERIC_KIND,SYSCOMP_KIND,ROUTINE_KIND:
        ...
        writeln('TYP není inicializován');
      END
